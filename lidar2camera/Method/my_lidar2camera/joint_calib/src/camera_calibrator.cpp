...
void CameraCalibrator::set_input(
  ...
  std::cout << "lidar_datas:string->float" << std::endl;
  ...
  for (const auto &img : vec_mat_) {
    ...
    i++;
    //迭代模板：一个是类型，第二个参数为迭代的最大次数，第三个是特定的阈值
...
void CameraCalibrator::get_result(cv::Mat &camera_matrix, cv::Mat &k,
  ...
  for (size_t i = 0; i < rvecsMat.size(); i++) {
    ...
    std::cout << imgpoints_cir1 << std::endl;
    ...
    RT.block<3, 1>(0, 2) = r2;
    RT.block<3, 1>(0, 3) = t;
    vec_extrinsics.push_back(RT);     // 相机外参；
    std::cout << i << " RT : " << std::endl << RT << std::endl;  
  }
  ...
  for (size_t i = 0; i < _imgs_pts_cir2D_true.size(); i++) {
    ...
    // left bottom;
    lidar_point_pair.lidar_2d_point[2] = _imgs_pts_cir2D_true[i][2];
    lidar_point_pair.lidar_3d_point[2] = lidar_3d_pts_[i][2];

    // right bottom
    lidar_point_pair.lidar_2d_point[3] = _imgs_pts_cir2D_true[i][3];
    lidar_point_pair.lidar_3d_point[3] = lidar_3d_pts_[i][3];

    
    if (true) { // Show real LiDAR pixels
    ...
    if (i > 20) // no corresponding circle center for more than 20 ，后20张照片用做相机标定
    {
      continue;
    }
  ...
  double E = 0;
  double E_theta = 0;
  std::vector<double> picture_reproject;
  std::vector<double> picture_theta;
  std::vector<double> picture_sub;


  if (true) // Show the optimized projection effect
  {
    //prepare revc_R and tevc_R
    cv::Mat lidar2camera_R(3, 3, CV_64FC1);
    cv::Mat rvec_R(3, 1, CV_64FC1);
    cv::Mat tvec_R(3, 1, CV_64FC1);
    cv::Mat m;

    for(int i = 0; i < 3; i++){
      for(int j = 0; j < 3 ; j ++){
        lidar2camera_R.at<double>(i, j) = initial_extrinsic(i, j);
      }
      tvec_R.at<double>(0,i) = initial_extrinsic(3, i);
    }
    cv::Rodrigues(lidar2camera_R, rvec_R);
    std::cout << "lidar_to_camera RT:" << std::endl << initial_extrinsic << std::endl;
    std::cout << "lidar_to_camera R:" << std::endl << rvec_R << std::endl;
    std::cout << "lidar_to_camera T:" << std::endl << tvec_R <<std::endl;
    cv::Rodrigues(rvec_R, m);
    std::cout << "lidar_to_camera mat:" << std::endl << m << std::endl;
    ...
    for (size_t i = 0; i < lidar_point_pairs.size(); i++) {
      ...
      if(i == 0){
        std::cout << "lidar_3d: " << lidar_point_pairs[i].lidar_3d_point[0] << std::endl;
        std::cout << "pre_lidar_2d_point" << img_pt << std::endl;
      }
      ...
      if(i == 0){
        std::cout << "lidar_3d: " << lidar_point_pairs[i].lidar_3d_point[1] << std::endl;
        std::cout << "pre_lidar_2d_point" << img_pt << std::endl;
      }
      ...
      if(i == 0){
        std::cout << "lidar_3d: " << lidar_point_pairs[i].lidar_3d_point[2] << std::endl;
        std::cout << "pre_lidar_2d_point" << img_pt << std::endl;
      }
      ...
      if(i == 0){
        std::cout << "lidar_3d: " << lidar_point_pairs[i].lidar_3d_point[3] << std::endl;
        std::cout << "pre_lidar_2d_point" << img_pt << std::endl;
      }
      //
      //
      ...
      if( i == 0 ){
        ...
        std::cout << "第一个圆心的雷达3d坐标" << lidar_point_pairs[i].lidar_3d_point[i] << std::endl;
        std::cout << "第一个圆心的雷达映射像素坐标" << lidar_point_pairs[i].lidar_2d_point[i] << std::endl;
        // std::cout << "pre_lidar_2d_point" << img_pt << std::endl;
        std::cout << "雷达to相机外参:" << std::endl << initial_extrinsic << std::endl;
        std::vector<cv::Point2f> tmp;
        cv::projectPoints(_boards_pts_cir[0], rvecsMat[i], tvecsMat[i], camera_matrix, k, tmp);
        std::cout << "棋盘格中四个圆心的世界坐标:" << _boards_pts_cir[i] << std::endl;
        std::cout << "四个圆心通过相机重投影的坐标:" << tmp[0] << std::endl;
        std::cout << "四个圆心通过相机重投影的坐标:" << tmp[1] << std::endl;
        std::cout << "四个圆心通过相机重投影的坐标:" << tmp[2] << std::endl;
        std::cout << "四个圆心通过相机重投影的坐标:" << tmp[3] << std::endl;
        std::cout << "优化后的相机外参:" << std::endl << vec_extrinsics[0] << std::endl;

        // std::cout << "R:" << std::endl << rvecsMat[i] << std::endl;
        // std::cout << "T:" << std::endl << tvecsMat[i] << std::endl;       
        // cv::Mat rot;
        ...
        // r1 << rot.at<double>(0, 1), rot.at<double>(1, 1), rot.at<double>(2, 1);
        // r2 << rot.at<double>(0, 2), rot.at<double>(1, 2), rot.at<double>(2, 2);
        // t << tvecsMat[i].at<double>(0, 0), tvecsMat[i].at<double>(0, 1), tvecsMat[i].at<double>(0, 2);
        // Eigen::MatrixXd RT(3, 4);
        // RT.block<3, 1>(0, 0) = r0;
        // RT.block<3, 1>(0, 1) = r1;
        // RT.block<3, 1>(0, 2) = r2;
        // RT.block<3, 1>(0, 3) = t;
        // std::cout << i << " 优化前的RT : " << std::endl << RT << std::endl;

        

        cv::Point2f tt;
        lidar_projection(camera_intrinsic, vec_extrinsics[0], _boards_pts_cir[0][0], tt);
        std::cout << "pre:" << std::endl << tt << std::endl;
        std::cout << "优化后的相机内参:" << camera_intrinsic << std::endl;

      }


      std::vector<cv::Point2f> lidar_imgpoints_cir;
      std::vector<cv::Point3f> lidar_3d;
      for(int num = 0; num < 4; num++ ){
        lidar_3d.push_back(lidar_point_pairs[i].lidar_3d_point[num]);
      }
      cv::projectPoints(lidar_3d, rvec_R, tvec_R,
                      camera_matrix, k, lidar_imgpoints_cir);
      // cv::circle(undistort_img, lidar_imgpoints_cir[0], 8, (0, 255, 0), 13);
      // cv::circle(undistort_img, lidar_imgpoints_cir[1], 8, (0, 255, 0), 13);
      // cv::circle(undistort_img, lidar_imgpoints_cir[2], 8, (0, 255, 0), 13);
      // cv::circle(undistort_img, lidar_imgpoints_cir[3], 8, (0, 255, 0), 13);

      //计算误差
      std::cout << "第" << i + 1 << "帧数据:" << std::endl; 
      std::cout << "R:" << std::endl << rvecsMat[i] << std::endl;
      std::cout << "T:" << std::endl << tvecsMat[i] << std::endl; 
      cv::Mat rot;
      cv::Rodrigues(rvecsMat[i], rot);
      ...
      RT.block<3, 1>(0, 2) = r2;
      RT.block<3, 1>(0, 3) = t;
      std::cout << " 优化前的RT : " << std::endl << RT << std::endl;
        
      // for (int num = 0; num < 4; num ++){
      //   Eigen::Matrix<double, 4, 1> board_3d;
      //   Eigen::Matrix<double, 4, 1> lidar_3d;
      //   board_3d(0,0) = _boards_pts_cir[0][num].x;
      //   board_3d(1,0) = _boards_pts_cir[0][num].y;
      //   board_3d(2,0) = _boards_pts_cir[0][num].z;
      //   board_3d(3,0) = 1.0;
      //   lidar_3d(0,0) = lidar_point_pairs[i].lidar_3d_point[num].x;
      //   lidar_3d(1,0) = lidar_point_pairs[i].lidar_3d_point[num].y;
      //   lidar_3d(2,0) = lidar_point_pairs[i].lidar_3d_point[num].z;
      //   lidar_3d(3,0) = 1.0;
      //   Eigen::Matrix<double, 3, 1> board_tmp = vec_extrinsics[i] * board_3d;
      //   Eigen::Matrix<double, 3, 1> lidar_tmp = initial_extrinsic * lidar_3d;
      //   std::cout << "第" << num + 1 << "个" << "相机坐标系下棋盘格圆心坐标：(" << board_tmp.transpose() << ")" << std::endl;
      //   std::cout << "第" << num + 1<< "个" << "相机坐标系下雷达圆心坐标：(" << lidar_tmp.transpose() << ")" << std::endl;
      //   Eigen::Matrix<double, 2 ,1> board_standardization;
      //   board_standardization(0, 0) = board_tmp(0, 0)/board_tmp(2, 0);
      //   board_standardization(1, 0) = board_tmp(1, 0)/board_tmp(2, 0);
      //   Eigen::Matrix<double, 2 ,1> lidar_standardization;
      //   lidar_standardization(0, 0) = lidar_tmp(0, 0)/lidar_tmp(2, 0);
      //   lidar_standardization(1, 0) = lidar_tmp(1, 0)/lidar_tmp(2, 0);
      //   std::cout << "第" << num + 1 << "个" << "相机坐标系下棋盘格圆心标准化坐标：(" << board_standardization.transpose() << ")" << std::endl;
      //   std::cout << "第" << num + 1 << "个" << "相机坐标系下雷达圆心标准化坐标：(" << lidar_standardization.transpose() << ")" << std::endl;
      //   double error = 0;
      //   Eigen::Matrix<double, 2 ,1> D_value;
      //   D_value(0, 0) = board_standardization(0, 0)-lidar_standardization(0, 0);
      //   D_value(1, 0) = board_standardization(1, 0)-lidar_standardization(1, 0);
      //   error  = std::sqrt(D_value(0,0) * D_value(0,0) + D_value(1,0) * D_value(1,0));
      ...
      //   if(board_standardization(0, 0)/lidar_standardization(0, 0) >= lidar_standardization(0, 0)/board_standardization(0, 0)){
      //     D_value(0, 0) = board_standardization(0, 0)/lidar_standardization(0, 0);
      //   }
      //   else{
      //     D_value(0, 0) = lidar_standardization(0, 0)/board_standardization(0, 0);
      //   }

      //   if(board_standardization(1, 0)/lidar_standardization(1, 0) >= lidar_standardization(1, 0)/board_standardization(1, 0)){
      //     D_value(1, 0) = board_standardization(1, 0)/lidar_standardization(1, 0);
      //   }
      //   else{
      //     D_value(1, 0) = lidar_standardization(1, 0)/board_standardization(1, 0);
      //   }
      //   error  = D_value(0,0) + D_value(1,0);
      //   std::cout << "error :" << error << std::endl;
      //   E = E + error;
      // }
      double sing_image_sum_theta_error = 0;
      double sing_image_average_theta_error = 0;
      double sing_image_sum_sub = 0;
      double sing_image_average_sub = 0;
      std::ofstream of;
      std::string path = "results/datasets2/jpg_" + std::to_string(i+1) + "/" + std::to_string(i+1) + ".txt";
      of.open(path);
      for (int num = 0; num < 4; num ++){
        Eigen::Matrix<double, 4, 1> board_3d;
        Eigen::Matrix<double, 4, 1> lidar_3d;
        ...
        std::cout << "第" << num + 1 << "个" << "相机坐标系下棋盘格圆心坐标：(" << board_tmp.transpose() << ")" << std::endl;
        std::cout << "第" << num + 1<< "个" << "相机坐标系下雷达圆心坐标：(" << lidar_tmp.transpose() << ")" << std::endl;
        of << "第" << num + 1 << "个" << "相机坐标系下棋盘格圆心坐标：(" << board_tmp.transpose() << ")" << std::endl;
        of << "第" << num + 1<< "个" << "相机坐标系下雷达圆心坐标：(" << lidar_tmp.transpose() << ")" << std::endl;
        Eigen::Matrix<double, 2 ,1> board_standardization;
        board_standardization(0, 0) = board_tmp(0, 0)/board_tmp(2, 0);
        board_standardization(1, 0) = board_tmp(1, 0)/board_tmp(2, 0);
        Eigen::Matrix<double, 2 ,1> lidar_standardization;
        ...
        std::cout << "第" << num + 1 << "个" << "相机坐标系下棋盘格圆心标准化坐标：(" << board_standardization.transpose() << ")" << std::endl;
        std::cout << "第" << num + 1 << "个" << "相机坐标系下雷达圆心标准化坐标：(" << lidar_standardization.transpose() << ")" << std::endl;
        of << "第" << num + 1 << "个" << "相机坐标系下棋盘格圆心标准化坐标：(" << board_standardization.transpose() << ")" << std::endl;
        of << "第" << num + 1 << "个" << "相机坐标系下雷达圆心标准化坐标：(" << lidar_standardization.transpose() << ")" << std::endl;
        double error_sub = 0;
        Eigen::Matrix<double, 2 ,1> D_value;

        Eigen::Vector3f v1,v2;
        v1[0] = board_standardization(0, 0);
        v1[1] = board_standardization(1, 0);
        v1[2] = 1;
        v2[0] = lidar_standardization(0, 0);
        v2[1] = lidar_standardization(1, 0);
        v2[2] = 1;
        double cosValNew=v1.dot(v2) /(v1.norm()*v2.norm()); //角度cos值

        //消除浮点误差
        if (cosValNew > 1) cosValNew = 1 - (cosValNew - 1);

        double angleNew = acos(cosValNew) * 180 / M_PI;   //弧度角转角度

        std::cout << "角度:" << angleNew << "°" << std::endl;
        of << "角度:" << angleNew << "°" << std::endl;

        sing_image_sum_theta_error = sing_image_sum_theta_error + angleNew;

        if(std::abs(board_standardization(0, 0)/lidar_standardization(0, 0)) >= std::abs(lidar_standardization(0, 0)/board_standardization(0, 0))){
          D_value(0, 0) = std::abs(board_standardization(0, 0)/lidar_standardization(0, 0));
        }
        else{
          D_value(0, 0) = std::abs(lidar_standardization(0, 0)/board_standardization(0, 0));
        }

        if(std::abs(board_standardization(1, 0)/lidar_standardization(1, 0)) >= std::abs(lidar_standardization(1, 0)/board_standardization(1, 0))){
          D_value(1, 0) = std::abs(board_standardization(1, 0)/lidar_standardization(1, 0));
        }
        else{
          D_value(1, 0) = std::abs(lidar_standardization(1, 0)/board_standardization(1, 0));
        }
        error_sub  = D_value(0,0) + D_value(1,0);
        sing_image_sum_sub = sing_image_sum_sub + error_sub;
        std::cout << "error_sub :" << error_sub << std::endl;
        of << "error_sub :" << error_sub << std::endl;
      }
      of << std::endl;
      sing_image_average_theta_error = sing_image_sum_theta_error / 4;
      of << "单张图片平均角度误差: " << sing_image_average_theta_error << std::endl; 
      E_theta = E_theta + sing_image_sum_theta_error;
      picture_theta.push_back(sing_image_average_theta_error);

      sing_image_average_sub = sing_image_sum_sub / 4;
      of << "单张图片平均sub误差: " << sing_image_average_sub << std::endl; 
      E = E + sing_image_sum_sub;
      picture_sub.push_back(sing_image_average_sub);
      ...
      std::string save_name = "results/datasets2/jpg_" + std::to_string(i+1) + "/" + std::to_string(i+1) + ".jpg";
      cv::imwrite(save_name, undistort_img);
      double error = (error1 + error2 + error3 + error4) / 4;
      picture_reproject.push_back(error);
      lidar_reprojection_error += error;
      of << "单张图片平均重投影误差" << error << std::endl;
      number++;
      of.close();
    }
  }
  std::string path_results = "results/datasets2/results.txt";
  std::string path_reproject = "results/datasets2/reproject.txt";
  std::string path_theta = "results/datasets2/theta.txt";
  std::string path_sub = "results/datasets2/sub.txt";
  std::ofstream fr;
  std::ofstream ft;
  std::ofstream fs;
  fr.open(path_reproject);
  ft.open(path_theta);
  fs.open(path_sub);
  for(int i = 0; i < picture_reproject.size(); i++){
    fr << picture_reproject[i] << std::endl;
    ft << picture_theta[i] << std::endl;
    fs << picture_sub[i] << std::endl;
  }
  fr.close();
  ft.close();
  fs.close();
  std::ofstream ff;
  ff.open(path_results);

  std::cout << "lidar reprojection error: " << lidar_reprojection_error / number
            << std::endl;
  std::cout << "相机坐标系的重投影角度误差：" << E_theta / 4 / number << std::endl; 
  std::cout << "相机坐标系的重投影sub误差: " << E / 4 / number << std::endl;

  ff << "lidar reprojection error: " << lidar_reprojection_error / number
            << std::endl;
  ff << "相机坐标系的重投影角度误差：" << E_theta / 4 / number << std::endl; 
  ff << "相机坐标系的重投影sub误差: " << E / 4 / number << std::endl; 
  ff.close();
  
  std::cout << "lidar2camera RT:" << std::endl << initial_extrinsic << std::endl;
  std::cout << "camera intrinsic:" << std::endl << camera_intrinsic << std::endl;
  std::cout << "camera distortion:" << std::endl << k << std::endl;
...
void CameraCalibrator::make_board_points(const cv::Size &chessboard_size_) {
  ...
  vec_points_cir.emplace_back(1.0f, -4.0f, 0.0f);  // sim 左上角
  vec_points_cir.emplace_back(10.0f, -4.0f, 0.0f); // sim 右上角
  vec_points_cir.emplace_back(1.0f, 11.0f, 0.0f);   // sim 左下角
  vec_points_cir.emplace_back(10.0f, 11.0f, 0.0f);  // sim 右下角
...
void CameraCalibrator::refine_lidar2camera(
  ...
  // std::cout << "initial_extrinsic : " << initial_extrinsic << std::endl;
}
