...
void CameraCalibrator::get_result(cv::Mat &camera_matrix, cv::Mat &k,
  ...
  // for(int i = 0; i < lidar_point_pairs.size(); i++){
  //   std::cout << "board_3d_point" << _boards_pts_cir[i][0] << std::endl;
  //   std::cout << "board_3d_point" << _boards_pts_cir[i][1] << std::endl;
  //   std::cout << "board_3d_point" << _boards_pts_cir[i][2] << std::endl;
  //   std::cout << "board_3d_point" << _boards_pts_cir[i][3] << std::endl << std::endl;
  // }
  if (true) // Show the optimized projection effect
  {
    for (size_t i = 0; i < lidar_point_pairs.size(); i++) {
      ...
      if(1){
        ...
        std::cout << "gt_camera_2d_point" << pt << std::endl;
      }
      ...
      if(1){
        ...
        std::cout << "lidar_3d_point" << lidar_point_pairs[i].lidar_3d_point[i] << std::endl;
        std::cout << "lidar_2d_point" << lidar_point_pairs[i].lidar_2d_point[i] << std::endl;
        // std::cout << "pre_lidar_2d_point" << img_pt << std::endl;
        // std::cout << "initial_extrinsic:" << std::endl << initial_extrinsic << std::endl;
        std::vector<cv::Point2f> tmp;
        cv::projectPoints(_boards_pts_cir[i], rvecsMat[i], tvecsMat[i], camera_matrix, k, tmp);
        std::cout << "world_point :" << std::endl << _boards_pts_cir[i] << std::endl;
        std::cout << "pre_camera_2d_point" << tmp[0] << std::endl;
        std::cout << "pre_camera_2d_point" << tmp[1] << std::endl;
        std::cout << "pre_camera_2d_point" << tmp[2] << std::endl;
        std::cout << "pre_camera_2d_point" << tmp[3] << std::endl;
        std::cout << "camera_extrinsic:" << std::endl << vec_extrinsics[i] << std::endl;
        // std::cout << "R:" << std::endl << rvecsMat[i] << std::endl;
        // std::cout << "T:" << std::endl << tvecsMat[i] << std::endl;
        // cv::Point2f t;
        // lidar_projection(camera_intrinsic, vec_extrinsics[i], _boards_pts_cir[i][0], t);
        // std::cout << "pre:" << std::endl << t << std::endl;
      ...
      //   lidar_3d(3,0) = 1.0;
      //   Eigen::Matrix<double, 3, 1> board_tmp = RT * board_3d;
      ...
      std::ofstream of;
      std::string path = "results/datasets1/jpg_" + std::to_string(i+1) + "/" + std::to_string(i+1) + ".txt";
      of.open(path);
      for (int num = 0; num < 4; num ++){
        Eigen::Matrix<double, 4, 1> board_3d;
        Eigen::Matrix<double, 4, 1> lidar_3d;
        ...
        double angleNew = (cosValNew >= 1.0? 0.0  : acos(cosValNew)) * 180 / M_PI;;     //弧度角
        // printf("A: %.10lf\r\n", cosValNew);
        // std::cout << "v1:" << v1 << std::endl << "v2:" << v2 << std::endl;
        // std::cout << cosValNew << std::endl;
      ...
      std::string save_name = "results/datasets1/jpg_" + std::to_string(i+1) + "/" + std::to_string(i+1) + ".jpg";
      cv::imwrite(save_name, undistort_img);
  ...
  std::string path_results = "results/datasets1/results.txt";
  std::string path_reproject = "results/datasets1/reproject.txt";
  std::string path_theta = "results/datasets1/theta.txt";
  std::string path_sub = "results/datasets1/sub.txt";
  std::ofstream fr;
  std::ofstream ft;
  std::ofstream fs;
  ...
  std::cout << "lidar reprojection error: " << lidar_reprojection_error / number
            << std::endl;
  std::cout << "相机坐标系的重投影误差：" << E / 4 / number << std::endl; 
  std::cout << distort << std::endl;
  std::cout << tmp_dist << std::endl;
