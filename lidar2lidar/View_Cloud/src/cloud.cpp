...
string txt_path1 = "../Points/target_points"; 
string txt_path2 = "../Points/input_points"; 
string str;
...
int main(int argc, char **argv)
{
    std::string filename;
    std::cout << "plase choose:" << std::endl;
    std::cout << "1(target_cloud)    2(input_cloud)" << std::endl;
    if(argc <= 1){
        while( cin>>str ){
            if(str != "1"|| str != "2")
    ...
    if( str == "1" ){
        filename = "../data/room_scan1.pcd";
    }
    else if( str == "2"){
        filename = "../data/room_scan2_transformed.pcd";
    }

    std::cout << filename << std::endl;
    
    // std::string filename("../data/p64.pcd");
    // std::string filename("../data/room_scan1.pcd");
    // std::string filename("../data/room_scan2_transformed.pcd");

    string txt_name;
    if(filename == "../data/room_scan1.pcd"){
        txt_name = txt_path1 + ".txt";
    }
    else{
        txt_name = txt_path2 + ".txt";
    }
