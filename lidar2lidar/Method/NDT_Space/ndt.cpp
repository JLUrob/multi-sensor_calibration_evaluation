...
#include <pcl/registration/ndt.h> //配准所需头文件
...
int main()
{
	...
	PointCloudT::Ptr target_cloud(new PointCloudT);  // Original point cloud  （目标点云）
    PointCloudT::Ptr source_cloud(new PointCloudT);  // Original point cloud  （从文件读取待配准的点云）

	//时差=time.tok-time.tic 用于计时
	pcl::console::TicToc time;
	time.tic();
	std::string filename1 = "../data/room_scan1.pcd";//点云文件名
	if (pcl::io::loadPCDFile<pcl::PointXYZ>(filename1, *target_cloud) == -1)//*打开点云文件	
	{
		PCL_ERROR("Couldn't read file test_pcd.pcd\n");//输出错误原因：读取文件失败
		return(-1);//程序结束
	}
    //输出读取点云的点数与点云读取时间
	std::cout << "\nLoaded file " << filename1 << " (" << target_cloud->size() << " points) in " << time.toc() << " ms\n" << std::endl;
    
    std::string filename2 = "../data/room_scan2.pcd";//点云文件名
	if (pcl::io::loadPCDFile<pcl::PointXYZ>(filename2, *source_cloud) == -1)//*打开点云文件	
	{
		PCL_ERROR("Couldn't read file test_pcd.pcd\n");//输出错误原因：读取文件失败
		return(-1);//程序结束
	}
	//输出读取点云的点数与点云读取时间
	std::cout << "\nLoaded file " << filename2 << " (" << source_cloud->size() << " points) in " << time.toc() << " ms\n" << std::endl;
	...
	*/
    ...
    PointCloudT::Ptr downsampled_source(new PointCloudT), downsampled_target(new PointCloudT);
	PointCloudT::Ptr align_result(new PointCloudT(*source_cloud)), downsampled_align(new PointCloudT());
	//体素降采样滤波
	pcl::VoxelGrid<pcl::PointXYZ> voxel_grid;//创建滤波器对象
	voxel_grid.setLeafSize(0.1, 0.1, 0.1);//设置体素大小
	voxel_grid.setInputCloud(source_cloud);//输入待降采样的点云
	voxel_grid.filter(*downsampled_source);//降采样后放入cloud_icp_in
	std::cout << "down size *cloud_in_tgt to" << downsampled_source->size() << endl;
	*downsampled_align = *downsampled_source;

	//同理将另一个点云降采样
	voxel_grid.setInputCloud(target_cloud);
	voxel_grid.filter(*downsampled_target);
	std::cout << "down size *cloud_toberegistration to" << downsampled_target->size() << endl;
	...
	// Visualization 可视化 
	pcl::visualization::PCLVisualizer viewer("NDT demo");//窗口标题
	...
	pcl::visualization::PointCloudColorHandlerCustom<PointT> cloud_in_color_h(target_cloud, (int)255 * txt_gray_lvl, (int)255 * txt_gray_lvl, (int)255 * txt_gray_lvl);
	//在3个窗口中添加目标点云，目标点云在可视化的过程中是不会动的
	viewer.addPointCloud(target_cloud, cloud_in_color_h, "cloud_in_v1", v1);//显示cloud_in_tgt第一次的点云
	viewer.addPointCloud(target_cloud, cloud_in_color_h, "cloud_in_v2", v2);
	viewer.addPointCloud(downsampled_target, cloud_in_color_h, "cloud_in_v3", v3);

	//Transformed point cloud is green  经目标点云旋转平移后并且未降采样的点云设置为绿色，放在窗口1中
	pcl::visualization::PointCloudColorHandlerCustom<PointT> cloud_tr_color_h(align_result, 20, 180, 20);
	viewer.addPointCloud(align_result, cloud_tr_color_h, "cloud_tr_v1", v1);

	// ICP aligned point cloud is red  经旋转平移后未降采样的点云设置为红色，放在窗口2，后面将随着迭代过程更新
	pcl::visualization::PointCloudColorHandlerCustom<PointT> cloud_icp_color_h(align_result, 180, 20, 20);
	viewer.addPointCloud(align_result, cloud_icp_color_h, "cloud_ndt_v2", v2);

	// //ICP aligned point cloud is red  旋转平移并降采样的点云设置为红色，放在窗口3，用于icp的输入，并随着迭代过程更新位置
	viewer.addPointCloud(downsampled_align, cloud_icp_color_h, "cloud_ndt_v3", v3);
	...
	std::stringstream ss;
	ss << iterations;
	std::string iterations_cnt = "NDT iterations = " + ss.str();//显示算法已经迭代了多少次
	...
	ndt.setMaximumIterations(iterations);//设置最大迭代次数，iterations在上面已经设置为1，即只使用一次icp算法
	ndt.setInputSource(downsampled_source);//输入目标点云，是原读取的目标点云只经过降采样后的点云
	ndt.setInputTarget(downsampled_target);//输入待配准点云，是原读取的目标点云经旋转平移并降采样后的点云
	ndt.align(*downsampled_align);//经1次ndt算法配准后的点云

	Eigen::Matrix4f icp_trans = ndt.getFinalTransformation();//获取当前求解的旋转平移矩阵

	//使用得到的变换对未降采样的点云进行变换，以达到显示效果
	pcl::transformPointCloud(*source_cloud, *align_result, icp_trans);
	...
	if (ndt.hasConverged())//算法是否正常收敛
	{
		std::cout << "\nNDT has converged, score is " << ndt.getFitnessScore() << std::endl;//算法的mse（均方误差）
	...
	//在2、3窗口更新上次经ndt算法后的点云，这个地方注意看，之前是addpointcloud添加点云，这里是updatePointCloud是更新之前添加好的点云
	viewer.updatePointCloud(align_result, cloud_icp_color_h, "cloud_ndt_v2");
	viewer.updatePointCloud(downsampled_align, cloud_icp_color_h, "cloud_ndt_v3");
	...
	while (!viewer.wasStopped())
	{
        ...
        if (if_or_save)
        {
            pcl::io::savePCDFileASCII ("../transformed_data/room_scan2_transformed.pcd", *align_result);
        }
		if (next_iteration)
		{
			// The Iterative Closest Point algorithm  
			time.tic();
			ndt.align(*downsampled_align, icp_trans);//将上一次配准后的点云作为输入，进行下一次配准
			std::cout << "Applied 1 NDT iteration in " << time.toc() << " ms" << std::endl;

			icp_trans = ndt.getFinalTransformation();
